...
export default function BoatAiApp() {
  ...
  useEffect(() => {
    ...
    if (rawRace) {
      ...
      setActiveModelId(data.activeModelId || 'supervisor')
    }
  }, [])
  ...
  const activeModel = activeModelId === 'supervisor' ? supervisor.model : aiModels.find(m => m.id === activeModelId) || aiModels[4]
  const predictions = useMemo(() => makePredictions(boats, context, activeModel), [boats, context, activeModel])
    ...
    <main className="wrap">
      ...
      {stage === 'predict' && (
        <section>
          ...
          <div className="sectionTitle">AIモデル</div>
          <div className="card">
            <div className="formGrid">
              <div>
                <label>使用するAI</label>
                <select value={activeModelId} onChange={e => setActiveModelId(e.target.value)}>
                  <option value="supervisor">AI監督に任せる</option>
                  {aiModels.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                </select>
              </div>
              <div>
                <label>現在採用</label>
                <div className="rank">{activeModel.name}</div>
                <div className="small">{activeModel.description}</div>
              </div>
              <div>
                <label>AI監督の理由</label>
                <div className="small">{supervisor.reason}</div>
              </div>
            </div>
            <div className="sectionTitle">AIリーグ</div>
            <div className="tableWrap">
              <table><thead><tr><th>AI</th><th>本命</th><th>1着率</th><th>最高EV</th><th>買い候補</th><th>信頼度</th><th>理由</th></tr></thead>
                <tbody>{modelSummaries.map(m => <tr key={m.model.id} className={m.model.id === activeModel.id ? 'top' : ''}>
                  <td className="rank">{m.model.name}</td><td>{m.topLane}号艇 {m.topName}</td><td>{(m.topFirstRate * 100).toFixed(1)}%</td><td>{m.maxEv.toFixed(0)}</td><td>{m.buyCount}</td><td>{m.confidence.toFixed(0)}</td><td className="small">{m.reason}</td>
                </tr>)}</tbody>
              </table>
            </div>
          </div>
