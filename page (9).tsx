...
import ModelLeaguePanel from '@/components/ModelLeaguePanel';
import PurchasePlanPanel from '@/components/PurchasePlanPanel';
import RaceTimeline from '@/components/RaceTimeline';
import RaceMemo from '@/components/RaceMemo';
...
export default async function RacePage({params}:{params:{venueId:string;raceNo:string}}){
  ...
  return <>
    ...
    <div className="grid two" style={{marginTop:16}}><RaceTimeline deadline={race.deadline}/><DataQualityPanel race={race}/></div>
    ...
    <div className="grid two" style={{marginTop:16}}><ModelLeaguePanel race={race} predictions={predictions}/><PurchasePlanPanel picks={picks}/></div>
    <h2 className="section-title">オッズ調整</h2><OddsEditor picks={picks}/>
    <div className="grid two" style={{marginTop:16}}><ResultForm raceId={race.id} bestCombo={picks[0]?.combo||''}/><RaceMemo raceId={race.id}/></div>
  </>}
