...
export function Dashboard() {
  ...
  useEffect(() => {
    const load = () => setRaces(JSON.parse(localStorage.getItem('boat-ai-races') || '[]'));
    load();
    window.addEventListener('boat-ai-storage-updated', load);
    return () => window.removeEventListener('boat-ai-storage-updated', load);
  }, []);
