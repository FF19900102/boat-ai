...
export default function Page() {
  ...
  const trifecta = useMemo(() => generateTrifecta(predictions), [predictions]);
  const topRows = trifecta.slice(0, 12);
    ...
    <main className="page">
      ...
      <section className="grid grid3" style={{ marginTop: 16 }}>
        ...
        <div className="card">
          ...
          <div className="small">投資 {totalInvestment.toLocaleString()}円 / 回収 {totalPayout.toLocaleString()}円</div>
        </div>
      </section>
      ...
      <section className="grid grid2" style={{ marginTop: 16 }}>
        ...
        <div className="card">
          ...
          <table>
            <thead><tr><th>買い目</th><th>確率</th><th>推定オッズ</th><th>EV</th><th>判定</th></tr></thead>
            <tbody>
              {topRows.map((r) => (
                <tr key={r.key}><td><b>{r.key}</b></td><td>{r.probability}%</td><td>{r.odds}</td><td>{r.ev}</td><td><span className={`badge ${r.ev >= 120 ? 'buy' : r.ev >= 100 ? 'watch' : 'skip'}`}>{r.rank}</span></td></tr>
              ))}
            </tbody>
          </table>
        </div>
